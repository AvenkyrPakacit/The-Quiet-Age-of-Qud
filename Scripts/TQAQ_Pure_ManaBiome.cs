...
public class SlimyBiome : IBiome
{
	...
	public override int GetBiomeValue(string ZoneID)
	{
		...
		string objectTypeForZone = ZoneManager.GetObjectTypeForZone(num, num2, "TQAQ_Raseke");
	...
	{
		return IBiome.MutateZoneNameWith(Name, GetBiomeValue(ZoneID), "mana patch", "mana soaked", "mana field", "mana-drenched", "mana bog", null);
	}
	public override void MutateZone(Zone Z)
	{
		...
		new LiquidPools().BuildZone(Z, "TQAQ_Mana_Pool_Deep", 3 * GetBiomeValue(Z.ZoneID) - 4, "0", "Slimy Plants");
	}
}